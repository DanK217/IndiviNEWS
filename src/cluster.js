 ...
 *     ただし言語ゲートを通す: 日本語⇔英語のような言語をまたぐペアは
 *     あいまい一致の対象にしない（同一話題の言語横断検出はスコープ外）。
 *
...
// タイトルにCJK（ひらがな・カタカナ・漢字）が含まれるか。日本語と英語では
// 文字バイグラムの重なりが構造的に起きないため、言語をまたいだあいまい一致は
// スコープ外とし、同一言語クラスのペアにのみDice比較を許す（言語ゲート）。
const CJK_RE = /[\p{sc=Hiragana}\p{sc=Katakana}\p{sc=Han}]/u;
function hasCjk(title) {
  return CJK_RE.test(String(title));
}
...
function clusterItems(items) {
  ...
  const cjkFlags = items.map((item) => hasCjk(item.title));
